"""Bo3 sweep: Tokyo Prowess vs the 15-archetype Standard field (TRUE post-board).

Mirrors test_looting_bo3.py but for the Worldly Counsel Tokyo Prowess APL.
Uses MATCHUP_SB_PLANS when available. Reports match WR per matchup +
field-weighted match WR.

Run with: python scripts/test_tokyo_prowess_bo3.py
"""
import sys, os, time
...
US_KEY = "izzetprowessstandardtokyo"
N_MATCHES = 200
...
def main():
    ...
    print(f"\n{len(jobs)} Bo3 jobs (Tokyo Prowess x {len(FIELD)} opps), N={N_MATCHES}/each\n")
    results = {}
    sb_used = {}
    t0 = time.time()
    ...
    if ps > 0:
        ...
        print(f"\nFW Match WR Tokyo Prowess: {fw:.1f}%  (coverage {ps:.1%}, {len(paired)}/{len(FIELD)})")
